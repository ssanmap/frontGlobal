...
const currentCode = computed<CurrencyCode>(() => {
  const slug = route.name?.toString().startsWith('precio-') ? (route.params.slug as string) : 'peso-chileno'
...
<template>
  <div class="relative" @keydown="onEsc">
    <!-- Solo la bandera (botón transparente) -->
    <button
      type="button"
